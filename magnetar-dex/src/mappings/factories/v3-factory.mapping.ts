...
export async function handleV3PoolCreated(log: PoolCreatedLog): Promise<void> {
  assert(log.args, '!log.args');
  // Destructure args
  const { pool, token0, token1, tickSpacing } = log.args;
  ...
  // Create pool
  const name = `Cl-${token0Entity.symbol}/${token1Entity.symbol}`;
  ...
  await createV3PoolDatasource({ address: pool });
}
